  ...
  val p = if (g.paraNum > 1) {
    new Area {
      ...
      val twGen = new Area {
        ...
        loopSeqIntt := (twLoopCntMsbPadIntt >> shiftCnt)
          .resize(log2Up(g.paraNum) bits)(0, log2Up(g.paraNum) bits)
          .asBits
    ...
    new Area {
      ...
      val subDut = new Area {
        val MsbPadLoopCnt = Cat(B"1'b0", loopCnt).asBits
        val LsbPadLoopCnt = Cat(loopCnt, B"1'b0").asBits
        val msbMux = new SeqMux(g.Log2NttPoints)
        msbMux.io.SeqA := MsbPadLoopCnt
        msbMux.io.SeqB := LsbPadLoopCnt
        msbMux.io.isNtt := io.isNtt
        msbMux.io.ohSeq := OH_shifter
        msbMux.io.thermalSeq := Thermal_shifter
      ...
      import twGen._
      io.idle := fsm.isActive(fsm.IDLE)
      io.RdAddrOri.payload := RegNext(
        Vec(ins0_Msb(ins0_Msb.high downto 1).asUInt, ins1_Msb(ins1_Msb.high downto 1).asUInt)
      )
      io.RdLsbOri.payload := RegNext(Vec(ins0_Msb.lsb.asUInt, ins1_Msb.lsb.asUInt))
      val rdy = RegNext(fsm.isActive(fsm.LOOP))
      io.RdAddrOri.valid := rdy
      io.RdLsbOri.valid := rdy
      io.TwBus.payload.twAddr := RegNext(twAddr.asUInt)
      io.TwBus.valid := RegNext(fsm.isActive(fsm.LOOP))
    }
  }
...
//  import twGen._
//  io.idle := fsm.isActive(fsm.IDLE)
//  io.RdAddrOri.payload := RegNext(Vec(ins0_Msb(ins0_Msb.high downto 1).asUInt, ins1_Msb(ins1_Msb.high downto 1).asUInt))
//  io.RdLsbOri.payload := RegNext(Vec(ins0_Msb.lsb.asUInt, ins1_Msb.lsb.asUInt))
//  val rdy = RegNext(fsm.isActive(fsm.LOOP))
//  io.RdAddrOri.valid := rdy
//  io.RdLsbOri.valid := rdy
...
//}
//
//object CtrlOptAddrp1 {
//  def apply(
//      start: Bool,
//      isNtt: Bool,
//      cfg: NttCfgParam
//  ): CtrlOptAddrp1 = {
//    val inst = new CtrlOptAddrp1(cfg)
//    import inst._
//    io.start := start
//    io.isNtt := isNtt
//    inst
