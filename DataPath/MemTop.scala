...
//    val WrDataArray = slave Flow Vec(UInt(g.width bits), g.BI)
  ...
  val AddrDelay = new Area {
    ...
    val AddrDelaySt1 = Delay(Cat(uAddrMux.io.AddrBus), g.BfuNttDelay + g.ramLatency)
    val IdxDelaySt1 = Delay(Cat(uAddrMux.io.IdxTrans), g.BfuNttDelay + g.ramLatency)
