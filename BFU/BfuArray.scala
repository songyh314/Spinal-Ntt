...
case class BfuArray(g: NttCfg2414, debug: Boolean = false) extends Component {
  val io = new Bundle {
    val isNtt = in Bool ()
  ...
  bfu.toSeq.zip(io.dataIn.toSeq).foreach { case (t1, t2) => t1.io.dataIn := t2 }
  io.dataOut.toSeq.zip(bfu.toSeq).foreach { case (t1, t2) => t1 := t2.io.dataOut }
...
object BfuArrayGenV extends App {
  SpinalConfig(mode = Verilog, nameWhenByFile = false, anonymSignalPrefix = "tmp", targetDirectory = "./rtl/Ntt/Bfu/")
    .generate(new BfuArray(NttCfg2414()))
}
